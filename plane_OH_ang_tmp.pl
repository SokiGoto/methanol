...
my @O_data; #0 step , 1 x coor, 2 y coor, 3 z coor
my @C_data; #0 step , 1 x coor, 2 y coor, 3 z coor
my @H_data; #0 step , 1 atom number , 2 x coor, 3 y coor, 4 z coor
            #migration H is atom number(row 1) = 1
my @OC_dicetance;
my @OH_dicetance;
...
for (my $i = 0; $i < $STEP; $i++){
	...
	for (my $j = $extra_line; $j < $Number_of_atoms + $extra_line; $j++){
			...
			if ($data[0] eq "H"){
				$H_data[$i][$H_count][1] = $data[1];
                $H_data[$i][$H_count][2] = $data[2];
                $H_data[$i][$H_count][3] = $data[3];
				$H_count += 1;
			}
	}
	...
	my @OH_y;
	my @OH_z;
	for (my $j = 0; $j < $H_count; $j++){
		$OH_x[$j] = $H_data[$i][$j][1]-$O_data[$i][1];
		$OH_y[$j] = $H_data[$i][$j][2]-$O_data[$i][2];
		$OH_z[$j] = $H_data[$i][$j][3]-$O_data[$i][3];
		$OH_dicetance[$i][$j] = sqrt($OH_x[$j]*$OH_x[$j] + $OH_y[$j]*$OH_y[$j] + $OH_z[$j]*$OH_z[$j]);
	}

	my $perpendicular_line_x = $OH_y[1]*$OC_z - $OH_z[1]*$OC_y;
	my $perpendicular_line_y = $OH_z[1]*$OC_x - $OH_x[1]*$OC_z;
	my $perpendicular_line_z = $OH_x[1]*$OC_y - $OH_y[1]*$OC_x;

	my $num = ($perpendicular_line_x*$OH_x[1] + $perpendicular_line_y*$OH_y[1] + $perpendicular_line_z*$OH_z[1])/(sqrt($perpendicular_line_x*$perpendicular_line_x + $perpendicular_line_y*$perpendicular_line_y + $perpendicular_line_z*$perpendicular_line_z)*sqrt($OH_x[1]*$OH_x[1] + $OH_y[1]*$OH_y[1] + $OH_z[1]*$OH_z[1]));
	my $num_2 = rad2deg(acos($num));
	#print "$num_2\n";
	
	$num = ($perpendicular_line_x*$OC_x + $perpendicular_line_y*$OC_y + $perpendicular_line_z*$OC_z)/(sqrt($perpendicular_line_x*$perpendicular_line_x + $perpendicular_line_y*$perpendicular_line_y + $perpendicular_line_z*$perpendicular_line_z)*sqrt($OC_x*$OC_x + $OC_y*$OC_y + $OC_z*$OC_z));
	$num_2 = rad2deg(acos($num));
	print "$num_2\n";

	print "O  $O_data[$i][1] $O_data[$i][2] $O_data[$i][3]\n";
	print "C  $C_data[$i][1] $C_data[$i][2] $C_data[$i][3]\n";
	print "H  $H_data[$i][1][1] $H_data[$i][1][2] $H_data[$i][1][3]\n";
	print "OH $OH_x[1] $OH_y[1] $OH_z[1]\n";
	print "OC $OC_x $OC_y $OC_z\n";
	print "pl $perpendicular_line_x $perpendicular_line_y $perpendicular_line_z\n";

	for (my $j = 0; $j < $H_count; $j++){
		my $num = ($perpendicular_line_x*$OH_x[$j] + $perpendicular_line_y*$OH_y[$j] + $perpendicular_line_z*$OH_z[$j])/(sqrt($perpendicular_line_x*$perpendicular_line_x + $perpendicular_line_y*$perpendicular_line_y + $perpendicular_line_z*$perpendicular_line_z)*sqrt($OH_x[$j]*$OH_x[$j] + $OH_y[$j]*$OH_y[$j] + $OH_z[$j]*$OH_z[$j]));
    	my $ang_deg = rad2deg(acos($num));
		if ($j == 3){
			print "$j  $ang_deg\n";
			print OUT "$i $j $ang_deg\n";
		}
	}
